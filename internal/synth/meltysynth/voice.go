...
func (v *voice) start(region regionPair, channel int32, key int32, velocity int32) {
	// A Voice can be reused after stealing or exclusive-class replacement.
	// Reset every mix/send accumulator before initializing the new state so no
	// values from the previous note leak into the new attack.
